...
def route_to_list(nav_path):
    ...
    for cur_waypoint, _ in nav_path:
        ...
        if prev_p != None:
            dist_prev = distance(prev_p.x, prev_p.y, x, y)
            if dist_prev < 0.1:
                continue
...
def get_frenet(x, y, path, s_map):
    ...
    if ind_closest < len(path):
        ...
        # Get the 2 points
        if use_previous:
            p1 = Point(path[ind_closest - 1].x, path[ind_closest - 1].y)
            p2 = Point(path[ind_closest].x, path[ind_closest].y)
            prev_idx = ind_closest - 1
        else:
            p1 = Point(path[ind_closest].x, path[ind_closest].y)
            p2 = Point(path[ind_closest + 1].x, path[ind_closest + 1].y)

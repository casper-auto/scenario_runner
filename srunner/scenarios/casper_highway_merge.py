...
class CasperHighwayMerge(BasicScenario):

    """
    This is a single ego vehicle scenario
    """
    category = "CasperHighwayMerge"
    ...
    def __init__(self, world, ego_vehicles, config, randomize=True, debug_mode=False,
                 ...
                 dense_traffic=False, cooperative_drivers=False, num_of_players=0, report_enable=False):
        """
        ...
        reference_location = carla.Location(16.4,
                                            200,
                                            config.trigger_points[0].location.z)
        self._reference_waypoint = self._map.get_waypoint(reference_location)
        ...
        super(CasperHighwayMerge, self).__init__("CasperHighwayMerge",
                                                 ego_vehicles,
                                                 config,
                                                 world,
                                                 debug_mode,
    ...
    def _initialize_actors(self, config):
        ...
        other_vehicle_a_waypoint, _ = get_waypoint_in_distance(self._reference_waypoint, self._other_vehicle_a_location, junction=True)
        other_vehicle_b_waypoint, _ = get_waypoint_in_distance(self._reference_waypoint, self._other_vehicle_b_location, junction=True)
        other_vehicle_c_waypoint, _ = get_waypoint_in_distance(self._reference_waypoint, self._other_vehicle_c_location, junction=True)
        other_vehicle_d_waypoint, _ = get_waypoint_in_distance(self._reference_waypoint, self._other_vehicle_d_location, junction=True)
        other_vehicle_e_waypoint, _ = get_waypoint_in_distance(self._reference_waypoint, self._other_vehicle_e_location, junction=True)
        other_vehicle_f_waypoint, _ = get_waypoint_in_distance(self._reference_waypoint, self._other_vehicle_f_location, junction=True)
        other_vehicle_g_waypoint, _ = get_waypoint_in_distance(self._reference_waypoint, self._other_vehicle_g_location, junction=True)
        other_vehicle_h_waypoint, _ = get_waypoint_in_distance(self._reference_waypoint, self._other_vehicle_h_location, junction=True)
        other_vehicle_i_waypoint, _ = get_waypoint_in_distance(self._reference_waypoint, self._other_vehicle_i_location, junction=True)
    ...
    def _create_behavior(self):
        ...
        target_location_others = carla.Location(15.1, -117.7, 1)

        target_lane_vehicle_a = py_trees.composites.Parallel(
                    "Target Lane Vehicle A",
                    policy=py_trees.common.ParallelPolicy.SUCCESS_ON_ONE)
        target_lane_vehicle_a.add_child(IDMHighwayMergeAgentBehavior(self.other_actors[0], target_location_others, self._other_vehicle_a_wp, cooperative_driver=self._cooperative_drivers, dense_traffic = self._dense_traffic))

        target_lane_vehicle_b = py_trees.composites.Parallel(
                    "Target Lane Vehicle B",
                    policy=py_trees.common.ParallelPolicy.SUCCESS_ON_ONE)
        target_lane_vehicle_b.add_child(IDMHighwayMergeAgentBehavior(self.other_actors[1], target_location_others, self._other_vehicle_b_wp,  cooperative_driver=self._cooperative_drivers, dense_traffic = self._dense_traffic))

        target_lane_vehicle_c = py_trees.composites.Parallel(
                    "Target Lane Vehicle C",
                    policy=py_trees.common.ParallelPolicy.SUCCESS_ON_ONE)
        target_lane_vehicle_c.add_child(IDMHighwayMergeAgentBehavior(self.other_actors[2], target_location_others, self._other_vehicle_c_wp, cooperative_driver=self._cooperative_drivers, dense_traffic = self._dense_traffic))

        target_lane_vehicle_d = py_trees.composites.Parallel(
                    "Target Lane Vehicle D",
                    policy=py_trees.common.ParallelPolicy.SUCCESS_ON_ONE)
        target_lane_vehicle_d.add_child(IDMHighwayMergeAgentBehavior(self.other_actors[3], target_location_others, self._other_vehicle_d_wp, cooperative_driver=self._cooperative_drivers, dense_traffic = self._dense_traffic))

        target_lane_vehicle_e = py_trees.composites.Parallel(
                    "Target Lane Vehicle E",
                    policy=py_trees.common.ParallelPolicy.SUCCESS_ON_ONE)
        target_lane_vehicle_e.add_child(IDMHighwayMergeAgentBehavior(self.other_actors[4], target_location_others, self._other_vehicle_e_wp, cooperative_driver=self._cooperative_drivers, dense_traffic = self._dense_traffic))

        target_lane_vehicle_f = py_trees.composites.Parallel(
                    "Target Lane Vehicle F",
                    policy=py_trees.common.ParallelPolicy.SUCCESS_ON_ONE)
        target_lane_vehicle_f.add_child(IDMHighwayMergeAgentBehavior(self.other_actors[5], target_location_others, self._other_vehicle_f_wp, cooperative_driver=self._cooperative_drivers, dense_traffic = self._dense_traffic))

        target_lane_vehicle_g = py_trees.composites.Parallel(
                    "Target Lane Vehicle G",
                    policy=py_trees.common.ParallelPolicy.SUCCESS_ON_ONE)
        target_lane_vehicle_g.add_child(IDMHighwayMergeAgentBehavior(self.other_actors[6], target_location_others, self._other_vehicle_g_wp, cooperative_driver=self._cooperative_drivers, dense_traffic = self._dense_traffic))

        target_lane_vehicle_h = py_trees.composites.Parallel(
                    "Target Lane Vehicle H",
                    policy=py_trees.common.ParallelPolicy.SUCCESS_ON_ONE)
        target_lane_vehicle_h.add_child(IDMHighwayMergeAgentBehavior(self.other_actors[7], target_location_others, self._other_vehicle_h_wp, cooperative_driver=self._cooperative_drivers, dense_traffic = self._dense_traffic))

        target_lane_vehicle_i = py_trees.composites.Parallel(
                    "Target Lane Vehicle I",
                    policy=py_trees.common.ParallelPolicy.SUCCESS_ON_ONE)
        target_lane_vehicle_i.add_child(IDMHighwayMergeAgentBehavior(self.other_actors[8], target_location_others, self._other_vehicle_i_wp, cooperative_driver=self._cooperative_drivers, dense_traffic = self._dense_traffic))
        ...
        # enable ego vehicle
        enable_ego_vehicle = py_trees.composites.Parallel(
            "Enable Ego Vehicle ",
            policy=py_trees.common.ParallelPolicy.SUCCESS_ON_ONE)
        enable_ego_vehicle.add_child(ActorTransformSetter(self.ego_vehicles[0], self._ego_vehicle_transform))

        # end condition
        endcondition = py_trees.composites.Parallel("Waiting for end position",
                                                    policy=py_trees.common.ParallelPolicy.SUCCESS_ON_ALL)

        endcondition_merged = DriveDistance(self.ego_vehicles[0], 90.0, "DriveDistance" )
        endcondition.add_child(endcondition_merged)

        # Build behavior tree
        sequence = py_trees.composites.Sequence("Sequence Behavior")
        sequence.add_child(enable_ego_vehicle)

        root = py_trees.composites.Parallel("Parallel Behavior", policy=py_trees.common.ParallelPolicy.SUCCESS_ON_ALL)
        root.add_child(driving_in_target_lane)
